...
const Form = () => {
  ...
  let DataObject=async()=>{
    const id = Math.floor(Math.random()*100) 
    let Data={id,title,date,value,description}
    ContextApi.SetDataa(prev=>[...prev,Data])
    console.log(Data);
    try {
     let res= await axios.post("https://expenses-app-32e19-default-rtdb.firebaseio.com/form.json",Data)
...
let FeatchData= async()=>{
 try {
   let respose=await axios.get("https://expenses-app-32e19-default-rtdb.firebaseio.com/form.json")
   let Data=[]
   for(let key in respose.data){
     let value = respose.data[key];
     value.id=key;
     Data.push(value);
   }
   ContextApi.SetDataa(Data)
 } catch (error) {
  console.log(error);
 }
}
